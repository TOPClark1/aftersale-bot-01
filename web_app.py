...
def _render_page(values=None, result=None, log_output=""):
    ...
    if result:
        ...
        status = "运行成功" if result["ok"] else "运行失败"
        status_html = f"""
...
<head>
  ...
  <style>
    ...
    .btn {{ margin-top: 16px; background: #2563eb; color: #fff; border: none; border-radius: 8px; padding: 12px 16px; cursor: pointer; font-size: 15px; }}
...
<body>
  <div class=\"container\">
    ...
    <p class=\"desc\">在这里填写邮箱、数据库、模板和语气要求，然后直接点击运行，无需手工写命令。</p>
    ...
    <form method=\"post\">
      ...
      <button class=\"btn\" type=\"submit\">运行流水线</button>
    </form>
    {status_html}
  </div>
</body>
